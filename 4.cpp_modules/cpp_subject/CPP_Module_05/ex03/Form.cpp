...
Form::Form()
    :   _name("NONAME"),  _type("NO"), _isSigned(false), _signedGrade(1), _exeGrade(1)
...
Form::Form(const Form& f)
    :   _name(f.getName()) , _type(f.getType()) , _isSigned(f.getIsSigned()), _signedGrade(f.getSignedGrade())
...
Form::Form(const std::string name, bool isSigned, const int sGrade, const int eGrade, const std::string type)
    :   _name(name), _type(type) , _isSigned(isSigned), _signedGrade(sGrade), _exeGrade(eGrade)
{
    if (_signedGrade > 150 || _exeGrade > 150)
        throw (GradeTooLowException());
    else if (_signedGrade < 1 || _exeGrade < 1)
...
void Form::beSigned(const Bureaucrat& b)
{
    ...
    if (b.getGrade() > _signedGrade)
        throw (GradeTooLowException());
    _isSigned = true;
    std::cout << b.getName() << " is Signed " << getName() <<" [" << getIsSigned() << "]" << std::endl;
}
